...
| Cloudflare の RUM ビーコン | Pages が `/cdn-cgi/rum` と `cloudflareinsights.com/beacon.min.js` を自動注入する設定になっている場合がある。**アクセス解析に該当するため、プライバシーポリシーの「解析は未導入」という記述と矛盾する。** 使わないなら Cloudflare 側で無効化し、使うならポリシーを改定する |
